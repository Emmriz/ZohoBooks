...
if (empty($perms['all'])) {
    ...
    header('Location: ' . APP_URL . '/modules/dashboard/index.php'); exit;
}

$currentModule = 'branding';
$pageTitle     = 'Branding & Theme';

// Upload dir relative to app root
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['logo'])) {
    ...
    if ($file['error'] !== UPLOAD_ERR_OK) {
        ...
        $_SESSION['flash_error'] = 'Invalid file type. Use PNG, JPG, SVG, or WebP.';
        ...
        if (move_uploaded_file($file['tmp_name'], $dest)) {
            saveSetting('logo_path', $uploadDirUrl . $filename);
            $_SESSION['flash_success'] = 'Logo uploaded successfully!';
        } else {
            $_SESSION['flash_error'] = 'Failed to save file. Check folder permissions.';
        }
    }
...
// ── Handle Logo Remove ───────────────────────────────────────
if (get('action') === 'remove_logo') {
    $old = getSetting('logo_path', '');
    if ($old && file_exists(__DIR__ . '/../../' . $old)) @unlink(__DIR__ . '/../../' . $old);
    saveSetting('logo_path', '');
    $_SESSION['flash_success'] = 'Logo removed.';
    header('Location: ' . APP_URL . '/modules/branding/index.php'); exit;
}

// ── Handle Theme Change ──────────────────────────────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST' && post('form') === 'theme') {
    saveSetting('brand_color', post('brand_color', 'blue'));
    saveSetting('app_name',    post('app_name', 'ZohoBooks'));
    $_SESSION['flash_success'] = 'Branding updated! Refresh to see changes.';
    header('Location: ' . APP_URL . '/modules/branding/index.php'); exit;
}

$currentThemeName = getSetting('brand_color', 'blue');
$currentLogo      = getSetting('logo_path', '');
$currentAppName   = getSetting('app_name', APP_NAME);

$themes = [
    'blue'   => ['name' => 'Ocean Blue',     'primary' => '#2563eb', 'sidebar' => '#0f172a', 'accent' => '#3b82f6'],
    'green'  => ['name' => 'Forest Green',   'primary' => '#16a34a', 'sidebar' => '#052e16', 'accent' => '#22c55e'],
    'purple' => ['name' => 'Royal Purple',   'primary' => '#7c3aed', 'sidebar' => '#2e1065', 'accent' => '#8b5cf6'],
    'red'    => ['name' => 'Crimson Red',    'primary' => '#dc2626', 'sidebar' => '#1c0606', 'accent' => '#ef4444'],
    'orange' => ['name' => 'Sunset Orange',  'primary' => '#ea580c', 'sidebar' => '#1c0a02', 'accent' => '#f97316'],
    'teal'   => ['name' => 'Teal',           'primary' => '#0d9488', 'sidebar' => '#042f2e', 'accent' => '#14b8a6'],
    'rose'   => ['name' => 'Rose',           'primary' => '#e11d48', 'sidebar' => '#1c0010', 'accent' => '#f43f5e'],
    'slate'  => ['name' => 'Slate Gray',     'primary' => '#475569', 'sidebar' => '#020617', 'accent' => '#64748b'],
];
...
<div class="max-w-4xl mx-auto space-y-6">
  ...
  <div class="card p-6">
    <div class="flex items-center gap-3 mb-5">
      <div class="w-9 h-9 rounded-xl flex items-center justify-center" style="background:var(--brand-light)">
        <i data-lucide="image" class="w-5 h-5" style="color:var(--brand)"></i>
      </div>
      <div>
        <h2 class="text-sm font-semibold text-gray-800">Company Logo</h2>
        <p class="text-xs text-gray-400">Shown in the sidebar. PNG, JPG, SVG or WebP — max 2MB.</p>
      </div>
    </div>
    ...
    <div class="flex items-start gap-8">
      ...
      <div class="flex-shrink-0">
        ...
        <div class="w-32 h-32 rounded-2xl border-2 border-dashed border-gray-200 flex items-center justify-center bg-gray-50 overflow-hidden">
          <?php if ($currentLogo): ?>
            <img src="<?= APP_URL ?>/<?= clean($currentLogo) ?>?t=<?= time() ?>"
                 alt="Logo" class="max-w-full max-h-full object-contain p-2">
          <?php else: ?>
        ...
        <?php if ($currentLogo): ?>
        <a href="?action=remove_logo"
           onclick="return confirm('Remove logo?')"
           class="mt-2 flex items-center justify-center gap-1 text-xs text-red-500 hover:text-red-700 transition-colors">
      ...
      <div class="flex-1">
        <form method="POST" enctype="multipart/form-data" id="logoForm">
          <div id="dropZone"
               class="border-2 border-dashed border-gray-200 rounded-2xl p-8 text-center cursor-pointer hover:border-gray-400 transition-colors"
          ...
          <div id="previewArea" class="hidden mt-4">
            <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
              <img id="previewImg" src="" alt="Preview" class="w-12 h-12 object-contain rounded-lg border border-gray-200 bg-white">
              <div class="flex-1 min-w-0">
                <p id="previewName" class="text-xs font-medium text-gray-700 truncate"></p>
            ...
            <button type="submit" class="btn-primary w-full justify-center mt-3">
  ...
  <div class="card p-6">
    <form method="POST">
      ...
      <div class="flex items-center gap-3 mb-5">
        <div class="w-9 h-9 rounded-xl flex items-center justify-center" style="background:var(--brand-light)">
          <i data-lucide="type" class="w-5 h-5" style="color:var(--brand)"></i>
        </div>
        <div>
          <h2 class="text-sm font-semibold text-gray-800">App Name & Color Theme</h2>
          <p class="text-xs text-gray-400">Customize the app name and pick your brand color.</p>
        </div>
      </div>

      <!-- App Name -->
      <div class="mb-6">
        <label class="form-label">App / Company Name</label>
        <input type="text" name="app_name" class="form-input max-w-sm"
               value="<?= clean($currentAppName) ?>" placeholder="Your Company Name">
      </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        ...
        <label class="cursor-pointer group">
          <input type="radio" name="brand_color" value="<?= $key ?>"
                 <?= $currentThemeName === $key ? 'checked' : '' ?>
                 class="sr-only peer">
          <div class="relative rounded-2xl overflow-hidden border-2 transition-all
                      peer-checked:border-gray-800 border-transparent
                      peer-checked:shadow-lg group-hover:border-gray-300">
            <!-- Sidebar preview -->
            <div class="h-16 flex">
      ...
      <div class="rounded-xl overflow-hidden border border-gray-200 mb-6">
        <div class="px-4 py-2 text-xs font-medium text-white flex items-center gap-2" id="previewBar" style="background:var(--brand)">
          <i data-lucide="eye" class="w-3.5 h-3.5"></i>
          Preview — button and accent colors will look like this
        </div>
        <div class="p-4 flex items-center gap-3 bg-gray-50">
          <button type="button" id="previewBtn" class="btn-primary text-xs" style="background:var(--brand)">
            Primary Button
          </button>
          <span class="text-xs font-medium" style="color:var(--brand)">Link text</span>
  ...
  <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm text-blue-700">
    <strong>Tip:</strong> After saving your theme, the page will reload and all colors across the app will update instantly — sidebar, buttons, accents, and highlights.
  </div>
...
// Live theme preview
const themeColors = <?= json_encode(array_map(fn($t) => $t['primary'], $themes)) ?>;
document.querySelectorAll('[name="brand_color"]').forEach(radio => {
  radio.addEventListener('change', function() {
    const color = themeColors[this.value] || '#2563eb';
